...
import L from 'leaflet';
import 'leaflet/dist/leaflet.css';
import { LocationIcon, CircleIcon, SleepIcon, GasStationIcon } from 'lucide-react';
...
// Custom icons
const createIcon = (color, iconComponent) => L.divIcon({
  html: `<div style="background:${color};width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:16px;border:3px solid white;box-shadow:0 2px 6px rgba(0,0,0,0.3)">${iconComponent}</div>`,
  iconSize: [32, 32],
  iconAnchor: [16, 16],
  className: '',
});

const currentIcon = createIcon('#3182ce', <LocationIcon size={16} color="white" />);
const pickupIcon = createIcon('#38a169', <CircleIcon size={16} color="white" />);
const dropoffIcon = createIcon('#e53e3e', <CircleIcon size={16} color="white" />);
const restIcon = createIcon('#d69e2e', <SleepIcon size={16} color="white" />);
const fuelIcon = createIcon('#805ad5', <GasStationIcon size={16} color="white" />);
